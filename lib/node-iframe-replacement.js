...
var isUrl = require('is-url'),              // module use to verify the sourceUrl is an actual URL
    ...
    request = require('request-promise-cache'),   // promise based request object
    NodeCache = require('node-cache'),    // auto expiring in memory caching collection
    cache = new NodeCache({ stdTTL: 5 }); // cache source HTML for 5 minutes, after which we fetch a new version
...
function resolveTemplate(sourceUrl) {
    ...
    return new Promise(function(resolve, reject) {
        ...
        console.log('resolveTemplate sourceUrl', sourceUrl);

        // if its a url and we have the contents in cache
        if (isUrl(sourceUrl) && false) { // cache.get(sourceUrl)
